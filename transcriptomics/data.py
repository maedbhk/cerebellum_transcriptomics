...
class DataSet: 
    ...
    def __init__(self):
        ...
        self.exact = True
        self.tolerance = 2
        self.ibf_threshold = 0.5 
        self.probe_selection = 'diff_stability'
        self.lr_mirror = False
        self.gene_norm = 'zscore' # default is 'srs'
        self.sample_norm = 'zscore' # default is 'srs'
        self.corrected_mni = True
        self.reannotated = True
        self.return_counts = True
        self.return_donors = True
        self.region_agg = 'donors' # other option is 'samples'
        self.agg_metric = 'mean'
        self.donors = 'all'
        self.data_dir = Defaults.RAW_DATA_DIR
        self.verbose = 1
        self.n_proc = 1
        self.return_samples = True # I added in this parameter to return an unaggregated dataframe (labelled samples x genes)
    ...
    def process_gene_expression(self, atlas='Yeo-17'):
        ...
        atlas_files = self._get_atlas_files(atlas)
        atlas_info = pd.read_csv(atlas_files.info)

        # get expression data
        dataframes, counts = abagen.get_expression_data(
            str(atlas_files['image']), atlas_info=self.atlas_info, exact=self.exact,
            tolerance=self.tolerance, ibf_threshold=self.ibf_threshold, 
            probe_selection=self.probe_selection, lr_mirror=self.lr_mirror, gene_norm=self.gene_norm,
            sample_norm=self.sample_norm, corrected_mni=self.corrected_mni, reannotated=self.reannotated,
            return_counts=self.return_counts, return_donors=self.return_donors, region_agg=self.region_agg,
            agg_metric=self.agg_metric, donors=self.donors, data_dir=self.data_dir, 
            verbose=self.verbose, n_proc=self.n_proc, return_samples=self.return_samples)
    ...
    def save_atlas_info(self, atlas):
    ...
    def save_thresholded_data(self, atlas, which_genes='top', percentile=1):
    ...
    def resample_to_mni(self, atlas): 
    ...
    def join_atlases(self, atlas_cortex="Yeo-7", atlas_cerebellum="Buckner-7", num_regs=7):
        ...
        nib.save(new_image, os.path.join(Defaults.ATLAS_TEMPLATE_DIR, f'Yeo-Buckner-{num_regs}.nii'))

    def _get_atlas_info(self, atlas):
    ...
    def _get_roi_labels(self, atlas):
    ...
    def _get_atlas_files(self, atlas):
        ...
        Args:
            ...
            need to be resampled.
        """
        outname_image = Defaults.ATLAS_TEMPLATE_DIR / f'{atlas}.nii'
        outname_info =  Defaults.ATLAS_TEMPLATE_DIR / f'{atlas}-info.csv'
    ...
    def _threshold_genes_ds(self, ds, which_genes, percentile): 
    ...
    def _get_differential_stability(self, atlas): # outputs results of differential stability analysis - correlations between genes across donors
